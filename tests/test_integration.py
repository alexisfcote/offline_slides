...
class integration_test(unittest.TestCase):
    ...
    def test_output_without_errors(self):
        export_to_offline_slides(pjoin(self.tmpdir.name, 'testnb.ipynb'))
        assert os.path.isfile(pjoin(self.tmpdir.name, 'testnb.slides.offline.html'))
        assert os.path.isdir(pjoin(self.tmpdir.name, 'ext/ajax/libs'))
        assert os.path.isdir(pjoin(self.tmpdir.name, 'ext/ajax/libs/font-awesome'))
        ...
        assert os.path.isdir(pjoin(self.tmpdir.name, 'ext/ajax/libs/require.js'))
        assert os.path.isdir(pjoin(self.tmpdir.name, 'ext/ajax/libs/reveal.js'))

    def test_offline_jupyter_html_without_errors(self):
        export_to_offline(pjoin(self.tmpdir.name, 'testnb.ipynb'), slides=False)
    ...
    def test_offline_jupyter_no_download(self):
        export_to_offline(pjoin(self.tmpdir.name, 'testnb.ipynb'), slides=True, no_download=True)
        assert os.path.isfile(pjoin(self.tmpdir.name, 'testnb.slides.offline.html'))
